...
export class FiltroSuperiorComponent implements OnInit, OnChanges {
  ...
  meses$: Observable<Mes[]> | undefined;
  filter: FilterOperacao = new FilterOperacao();
  anos: number[] = [];
  @Output() selecaoMes = new EventEmitter<Mes>();
  @Input() selectedMonth: string | number = '';
  ...
  selecionarMes($event: Mes) {
    this.selecaoMes.emit($event);
    this.selectedMonth = $event.mesInteiro;
  }
